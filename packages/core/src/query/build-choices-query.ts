 ...
 * them.
 *
 * Unconditioned by the request: no filters, no search, no drill-down window. A
 ...
 *
 * The caller's scope is the exception, because it is not part of the request:
 * "the whole table" means the whole table this caller can see. Without it the
 * filter would name values off rows they are not allowed to read.
 *
